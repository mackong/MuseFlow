...
/**
 * GET /api/me/posts — the authenticated user's own published posts.
 * Same projection as the public feed; viewer state populated.
 */
export async function GET(req: Request) {
  try {
    const user = await getCurrentUser();
    if (!user) throw new UnauthenticatedError("Sign in to view your posts");
